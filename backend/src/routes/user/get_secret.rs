...
pub async fn get_secret(
    ...
    let (secret, secret_salt) = web_block_unpacked(move || {
        ...
        let user: User = match users::users.find(uid).select(User::as_select()).get_result(&mut conn) {
            Ok(user) => user,
...
#[cfg(test)]
mod tests {
    use crate::{middleware::jwt::JwtMiddleware, routes::user::{get_secret::{get_secret, GetSecretResponse}, tests::TestUser}, tests::configure};
