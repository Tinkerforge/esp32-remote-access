...
pub async fn management(
    ...
    let allowed_user: AllowedUser = web_block_unpacked(move || {
        match AllowedUser::belonging_to(&user).filter(allowed_users::charger_id.eq(&data.id))
            .select(AllowedUser::as_select()).get_result(&mut conn) {
                Ok(user) => Ok(user),
                Err(NotFound) => {
                    Err(Error::UserIsNotOwner)
                }
                Err(_err) => {
    ...
    web_block_unpacked(move || {
        match diesel::update(chargers::chargers).filter(chargers::id.eq(allowed_user.charger_id)).set(chargers::last_ip.eq(Some(ip))).execute(&mut conn) {
            Ok(_) => Ok(()),
            Err(_err) => {
                Err(Error::InternalError)
            }
...
mod tests {
    ...
    async fn test_management() {
        ...
        let app = App::new().configure(configure).wrap(JwtMiddleware).service(management);
